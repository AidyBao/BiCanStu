...
#define ZXAPI_KEY   @"bicanobject"

@implementation ZXAPISign
...
+ (NSString *)signWithDictionary:(NSDictionary *)dict1 withToken:(NSString *)token{
    ...
    NSString *passWordStr = [jsonString stringByAppendingString:ZXAPI_KEY];
    NSString *resultString = nil;
    if (token) {
        resultString = [passWordStr stringByAppendingString:token];
    }
    //加密
    NSString *MD5String = [ZXStringUtils md5_16bit:resultString];
    return MD5String;
