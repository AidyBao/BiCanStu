...
- (void)viewDidLoad {
    ...
    [self.webView setBackgroundColor:[UIColor zx_subTintColor]];
    if (_urlString && _urlString.length) {
        [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:_urlString]]];
    }else{
        [ZXAlertUtils showAAlertWithTitle:@"提示" message:@"访问出错了" buttonText:@"确定" buttonAction:^{
            [self dismissAction];
        }];
    }
...
//    [self.loadingHUD stopAnimating];
    [ZXHUD MBHideForView:self.view animate:true];
    [ZXAlertUtils showAAlertWithTitle:@"提示" message:@"访问出错了" buttonTexts:@[@"返回",@"刷新"] buttonAction:^(int buttonIndex) {
        if (buttonIndex == 0) {
            [self dismissAction];
        }else{
            [self.webView reload];
