...
#define ZX_IMAGE_NODATA    @"ZXNoData"
#define ZX_IMAGE_NONETWORK @"ZXNoNetwork"
...
@interface ZXEmptyView ()
{
    ...
    UILabel      * lbInfo1;
    UILabel      * lbInfo2;
    ZXTintButton * btnAcion;
}

@property (nonatomic,assign) HEmptyType   currentType;;

@property (nonatomic,copy)   ZXRCallBack  callBack;;

@end

@implementation ZXEmptyView

+ (ZXEmptyView *)emptyViewForView:(UIView *)view{
    if (view) {
        ZXEmptyView * eView = nil;
        for (id aView in [view subviews]) {
            if ([aView isKindOfClass:[ZXEmptyView class]]) {
                eView = aView;
                break;
            }
        }
...
+ (void)showNoDataInView:(UIView *)view text1:(NSString *)text1 text2:(NSString *)text2 heightFix:(CGFloat)heightFix{
    if (view) {
        ZXEmptyView * emptyView = [self emptyViewForView:view];
        if (emptyView && emptyView.currentType != HEmptyTypeNoData) {
            [emptyView removeFromSuperview];
            emptyView = nil;
        }
        if (!emptyView) {
            emptyView = [[ZXEmptyView alloc] init];
//            emptyView = [[ZXEmptyView alloc] initWithFrame:CGRectMake(0, 0, view.frame.size.width, view.frame.size.height + heightFix)];
        }
...
+ (void)showNetworkErrorInView:(UIView *)view heightFix:(CGFloat)heightFix refreshAction:(ZXRCallBack)refreshAction{
    if (view) {
        ZXEmptyView * emptyView = [self emptyViewForView:view];
...
- (instancetype)init{
    if (self = [super init]) {
        [self setBackgroundColor:[UIColor zx_subTintColor]];
        ...
        btnAcion = [[ZXTintButton alloc] init];
...
- (void)setEmptyViewType:(HEmptyType)type{
    ...
    if (type == HEmptyTypeNoData) {
        [imgIcon setImage:[UIImage imageNamed:ZX_IMAGE_NODATA]];
        [lbInfo1 setFont:[UIFont zx_titleFontWithSize:14]];
        [lbInfo1 setTextColor:[UIColor zx_sub2TextColor]];
        [lbInfo2 setFont:[UIFont zx_titleFontWithSize:14]];
        [lbInfo2 setTextColor:[UIColor zx_sub2TextColor]];
        ...
        [imgIcon setImage:[UIImage imageNamed:ZX_IMAGE_NONETWORK]];
        [lbInfo1 setFont:[UIFont zx_titleFontWithSize:17]];
        [lbInfo1 setTextColor:[UIColor zx_textColor]];
        [lbInfo2 setFont:[UIFont zx_titleFontWithSize:14]];
        [lbInfo2 setTextColor:[UIColor zx_sub2TextColor]];
